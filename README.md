...
After target selection, operations run in target order. Read-only operations isolate a target's diagnostic; mutations retain earlier successful targets if a later target fails and return a non-zero overall result. Use `ki register add --repo <path-or-pattern>` to add selected physical KI roots to the local user register without applying repairs. A local `ki repo conform` also records each selected root first, even when its declaration or later conformance checks fail, so the register remains an inventory for repair and bulk maintenance rather than a compliance badge.
