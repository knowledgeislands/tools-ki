...
Schema 2 workspace groups contain ordered typed `repository` and `workspace` members. A nested workspace member expands that workspace's default group recursively; all effective repository leaves must remain physically contained by the selecting workspace, with cycles and duplicates refused before an operation starts. `ki workspace list` reports local and effective counts plus the workspace-relative path, direct or nested origin, `repo_code`, title, and description from each leaf's `[ki-repo]` metadata.
