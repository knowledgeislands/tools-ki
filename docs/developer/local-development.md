...
`src/commands/workspace.ts` owns the KI-only `ki workspace init`, `register`, `list`, `show`, `add`, and `remove` lifecycle. Its `.ki-workspace.toml` file has `schema = 1`, a required default group, and named `members` maps keyed by relative path; each entry has `kind = "repository"` or `kind = "workspace"`. Repository keys may be patterns and workspace keys may not. This is the sole workspace format; other schema versions and historical array or flat `repositories` group forms are rejected.
