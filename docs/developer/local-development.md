...
`src/context.ts` resolves the physical current working directory, executable installation mode, XDG KI paths, user home, and an optional ancestor KI repository. `src/core/repository.ts` owns repository target selection for every `ki repo` command: repeated `--repo <path-or-pattern>` values or a mutually exclusive `--workspace <group>` resolve and preflight a deterministic set. With neither selector, a direct-CWD regular `.ki-workspace.toml` selects its default group before a direct-CWD regular `.mgit-config.toml` selects standard repositories, nested `main/` checkouts, and `dir` containers from its `members` table; it ignores mGit `symlinks` and bare stores. Otherwise discovery searches upward for `.ki-config.toml`. No selector searches an ancestor workspace or `.mgit-config.toml`, and neither path treats the home directory or filesystem root as a repository selector.

`src/commands/workspace.ts` owns the KI-only `ki workspace init`, `register`, `list`, `show`, `add`, and `remove` lifecycle. Its `.ki-workspace.toml` file has `schema = 1`, a required default group, and named ordered `members` arrays whose `{ type, path }` entries explicitly distinguish repository paths or patterns from nested workspace directories. This is the sole workspace format; other schema versions and the historical flat `repositories` group form are rejected.

`src/core/workspace.ts` owns both physical registration and workspace selection. Registration performs a deterministic post-order walk without following symbolic links, treats a regular `.ki-config.toml` as a terminal repository leaf, preflights every container configuration, and then refreshes only default groups from deepest container to root. Selection recursively expands nested members through their default groups, requires every target to remain physically contained by the selecting workspace, and refuses cycles or duplicate repository leaves before returning a target set. `ki workspace list` uses that same expansion and reads identity metadata from each repository leaf, so its effective inventory cannot drift from repository-operation resolution.
