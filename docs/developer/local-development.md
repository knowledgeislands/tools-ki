...
`src/commands/workspace.ts` owns the KI-only `ki workspace init`, `register`, `list`, `show`, `add`, and `remove` lifecycle. Its `.ki-workspace.toml` file has `schema = 2`, a required default group, and named ordered `members` arrays whose entries explicitly distinguish repository paths or patterns from nested workspace directories.
