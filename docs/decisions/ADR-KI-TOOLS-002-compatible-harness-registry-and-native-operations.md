...
- The active implementation surface comprises `ki help`, `ki version`, `ki completions`, `ki acquire`, `ki bootstrap`, `ki dev`, `ki diag`, `ki doctor`, `ki harness`, `ki repo`, and `ki skill`. Planned commands remain visible in documentation and `ki(1)` but do not appear in HELP or completion until they have tests and release evidence.
