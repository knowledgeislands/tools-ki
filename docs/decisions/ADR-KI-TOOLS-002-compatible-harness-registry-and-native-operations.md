...
The package-management vocabulary is reserved as follows: `ki missing` and `ki outdated` report user status; `ki install`, `ki reinstall`, and `ki uninstall` manage named capabilities; `ki update` updates the executable and installed harnesses; and `ki repo upgrade` applies available compatible capability releases. These forms preserve the explicit user and `ki repo` activation boundary.
...
- The active implementation surface comprises `ki help`, `ki version`, `ki completions`, `ki acquire`, `ki bootstrap`, `ki dev`, `ki diag`, `ki doctor`, `ki harness`, `ki install`, `ki list`, `ki reinstall`, `ki repo`, `ki skill`, `ki uninstall`, and `ki update`. Planned commands remain visible in documentation and `ki(1)` but do not appear in HELP or completion until they have tests and release evidence.
