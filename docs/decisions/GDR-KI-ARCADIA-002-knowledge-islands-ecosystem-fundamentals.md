...
- `ki-agentic-harness` is the canonical source of reusable agentic capabilities: compatible harnesses, skills, agents, MCP wrappers, evals, and hooks. It defines the content and semantics that a KI-compatible harness provides; it does not implement the public `ki` executable or originate normative portable contracts.
- `tools-ki` is the canonical source of the `ki` executable platform. It implements harness installation, repository resolution, scoped runtime activation, registered native operation hosting, reporting, migration, and public command grammar. It consumes compatible harness artifacts but does not own their standards or define portable normative contracts.
...
- `ki-website` is the autonomous public publication layer. It vendors source-labelled material from the other four repositories so it can build and deploy independently, but publication never transfers canonical ownership.
...
|   harness and skill semantics -> tools-ki
