...
`src/commands/agora/` and `src/core/agora.ts` own declared-Agora inspection and opening. `ki agora list`, `show`, and `open` resolve named homes from registered repositories' `.ki-config.toml` declarations; the owner is a projection participant and every other member must reciprocally declare its consent.
