...
`src/context.ts` resolves the physical current working directory, executable installation mode, XDG KI paths, user home, and an optional ancestor KI repository. `src/core/repository/index.ts` owns repository target selection for every `ki repo` command: repeated `--repo <path-or-pattern>` values or a mutually exclusive `--agora <name>` resolve and preflight a deterministic set. An Agora selects its registered owner and reciprocally declared member repositories, each of which must be a physical KI repository. With neither selector, a direct-CWD regular `.mgit-config.toml` selects standard repositories, nested `main/` checkouts, and `dir` containers from its `members` table; it ignores mGit `symlinks` and bare stores. Otherwise discovery searches upward for `.ki-config.toml`. No selector searches an ancestor mGit configuration, and neither path treats the home directory or filesystem root as a repository selector.

`src/commands/agora/` and `src/core/agora.ts` own declared-Agora inspection, machine root projection, and opening. `ki agora list`, `show`, `roots`, and `open` resolve named homes from registered repositories' `.ki-config.toml` declarations; the owner is a projection participant and every other member must reciprocally declare its consent. `ki agora roots <name>` is read-only and defaults to newline-delimited absolute physical roots; `--null` (`-0`) selects its NUL-safe alternate encoding.

`src/commands/repo/roadmap.ts` and `src/core/work/items.ts` provide the read-only `ki repo roadmap list` inventory. They reuse the shared repository target set, accept only direct physical regular Markdown work-item files, parse the narrow canonical frontmatter contract, and isolate an inventory diagnostic to its selected repository. The command renders every selected repository it can inspect, then exits with status `1` if any roadmap or trade diagnostic occurred. It does not create, transition, accept, prune, or repair roadmap records; those lifecycle operations remain owned by harness process skills.
