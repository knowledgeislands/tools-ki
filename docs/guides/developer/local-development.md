...
Each public command or command group lives under `src/commands/`. A command owns Commander grammar, argument and option validation, adaptation from `KiContext`, terminal presentation, and exit translation. Shared terminal primitives live under `src/commands/presentation/`; repository-specific progress and reporting live under `src/commands/repo/presentation/`. Keep terminal wording, trees, tables, icons, progress bars, and elapsed-time rendering on this side of the boundary.
...
Repository selection, execution, progress events, and subprocess handling are grouped below `src/core/repository/`. Roadmap operations live below `src/core/work/`; Agora resolution lives below `src/core/agora/`; acquisition logic lives below `src/core/acquire/`. Local-client launch integrations live below `src/core/open-target/adapters/`; each adapter owns one target's process invocation, while the domain registry exposes supported identifiers to every opening command. The observable contracts remain in the [repository operations](../../specs/repository-operations.md) and [Agora](../../specs/agoras.md) specifications rather than being duplicated here.
