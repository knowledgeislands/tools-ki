...
- [ ] Create schema-one manifest records at `$XDG_STATE_HOME/ki/managed-artifacts/<uuid>.toml` for the finite harness-install `.install-*` family only. A record names its operation, lifecycle state, exact absolute staging path, and lock path.
- [ ] Acquire a per-record `mkdir` lock directory before atomically publishing the `creating` manifest and before creating the staging directory. Atomically replace the manifest as the installation advances; remove it only after the successful artifact cleanup.
- [ ] Have `ki manage cleanup` acquire each lock non-blockingly before reasoning about its manifest. An existing, unsafe, malformed, or otherwise unverifiable lock is a `live` refusal; it is never broken automatically.
- [ ] Treat a lock-free `creating` or `recoverable` record as interrupted-recoverable and retain `ki manage repair` as its recovery owner. Do not add a delete command or change its existing recovery decisions.
- [ ] Scan only managed-artifact manifests in lexical identifier order for the new report. Refuse foreign paths, symlinks, malformed or future-versioned records, and paths outside the approved harness-install roots.
- [ ] Exercise live locks, interruption, lock-free recovery records, foreign paths, symlinks, malformed records, and deterministic reporting through CLI contracts.
