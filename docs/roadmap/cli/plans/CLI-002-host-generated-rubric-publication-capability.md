...
`tools-ki` must expose validated rendered-publication evidence and guarded publication mechanics to a capability's rubric context without becoming the owner of a `ki-skills` criterion or finding.
...
- That standalone writer is outside repository CONFORM, so it cannot participate in the host's session-owned transaction, rollback, re-audit, or FIXED reporting.
...
- `src/core/rubric-render.ts` and focused rubric/session/transaction modules
...
5. `ki repo conform --skill ki-skills --repo <fixture> --dry-run` reports the derived write without changing files, real CONFORM publishes it transactionally, and a repeated dry-run proposes no publication write.
6. Malformed catalogues, unsafe targets, concurrent replacement, and write failure leave repository state unchanged or roll it back completely.
...
No compatibility path, generated-Markdown parser, duplicate renderer, host-created automatic finding, or relaxed installed-harness validation is permitted.
...
- Round 2 — mechanical: implement host loading, transaction, context injection, and CLI-contract tests; files: exclusive `tools-ki/src/**`; gate: focused standalone, audit, conform, rollback, and parity tests.
