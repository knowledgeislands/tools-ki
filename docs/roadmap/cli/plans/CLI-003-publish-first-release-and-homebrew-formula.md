...
`homebrew-tap` currently contains `Formula/mgit.rb` only.

There is no published `ki` tag, release, immutable source-archive checksum, or `Formula/ki.rb`.
...
5. Run `brew audit`, `brew test`, and a clean source installation of `knowledgeislands/tap/ki`; verify `ki --version` and `ki --help`, then remove only the test installation if it was created solely for verification.
6. Document `brew install knowledgeislands/tap/ki` in the CLI and tap user-facing documentation, including the relationship to the existing local installer.
...
5. CLI-002 remains an independent, unblocked acquisition plan.

## Dependencies / blocks

This plan has no governed-plan dependency edge and does not block CLI-002.
