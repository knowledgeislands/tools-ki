...
`prune [id]` removes every selected repository's canonical item whose status is exactly `done` when no ID is supplied; with an ID it requires exactly one selected repository and removes only that item. It leaves every other status and every non-canonical file untouched. Without `[horizon]`, `promote` moves one step toward `blocking` and `demote` moves one step toward `future`. With `[horizon]`, each command moves directly to the named horizon only in its respective direction. An item already at that directional endpoint is an error, as is a target that reverses or makes no movement.
...
- [x] Extend the `ki repo roadmap` grammar with `prune [id]`, `promote <id> [horizon]`, and `demote <id> [horizon]`, retaining `list` and the repository target-selection contract.
...
- [x] Implement `prune [id]` as a deterministic selected-repository sweep when omitted, or one exact completed record when named. Delete only validated records with `status: done`, report each deletion and a summary, and succeed with an explicit empty result when there is nothing to prune.
...
- [x] Cover the public contract through `run(args, context)`: every horizon transition, directional endpoint, invalid direct target, missing identifier, selected repository behavior, exact and sweep prune eligibility, no-op prune, failure atomicity, and unchanged non-roadmap content.
...
- `ki repo roadmap prune [id]` removes only `done` records, either as a selected-repository sweep or one exact item, and reports an empty sweep without mutation when none exist.
