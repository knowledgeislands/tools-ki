...
const renderItem = (item: RubricItem<unknown>): string => {
  const remediation = item.mechanical
    ? `\n  - _Remediation:_ ${item.mechanical.remediation.class}${item.mechanical.remediation.class === 'automatic' ? '' : ` â ${item.mechanical.remediation.guidance}`}`
    : ''
  const judgment = item.judgment
    ...
    : ''
  return `- **${item.code} [${classification(item)}] â ${item.title}** â ${item.description} (${item.sources.join(', ')})${remediation}${judgment}`
}
...
export const renderRubricMarkdown = (definition: SkillRubricDefinition<unknown>): string => {
  const contents = definition.families
    .map((family) => {
      const heading = `${family.code} â ${family.title}`
  ...
  const families = definition.families
    .map(
      (family) =>
        `## ${family.code} â ${family.title}\n\nâ [standard](${family.standard})\n\n${family.description}\n\n${family.items.map(renderItem).join('\n')}`
    )
    .join('\n\n')
...
# Generated rubric â ${definition.concern}
