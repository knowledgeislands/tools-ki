...
// The provider boundary remains local-only: this adapter never discovers or contacts ChatGPT.
...
const chatgptKepFiles = async (capture: Capture): Promise<readonly KepFile[]> => [
  ...(await listFiles(capture.assets)).map((path) => ({ path: `assets/${path}`, source: join(capture.assets, path) })),
  { path: 'relationships/native.jsonl', content: `${await normalisedRelationships(capture)}\n` },
  ...(await listFiles(capture.originals)).map((path) => ({
    path: `source/originals/${path}`,
    source: join(capture.originals, path)
  })),
  ...(await listFiles(capture.records)).map((path) => ({
    path: `source/records/${path}`,
    source: join(capture.records, path)
  }))
]

const calculatePayload = async (capture: Capture): Promise<KepPayload> => prepareKep(await chatgptKepFiles(capture))
...
const writeKep = async (capture: Capture, payload: KepPayload, output: string): Promise<void> => {
  await publishKep({
    directory: output,
    files: await chatgptKepFiles(capture),
    payload,
    metadata: [
      'format = "kep"',
