...
export const roadmapStatistics = (
  ...
  const hasFutureTimestamp = (item: WorkItem): boolean =>
    Date.parse(item.createdAt) > now || Date.parse(item.updatedAt) > now
  const futureTimestamps = items.filter(hasFutureTimestamp).map((item) => item.id)
  const usable = items.filter((item) => !hasFutureTimestamp(item))
  const ages = usable.map((item) => Math.floor((now - Date.parse(item.createdAt)) / 1000))
  const inactivity = usable.map((item) => Math.floor((now - Date.parse(item.updatedAt)) / 1000))
