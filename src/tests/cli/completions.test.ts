...
describe('[ki completions]', () => {
  test('renders zsh and bash completion scripts', async () => {
    ...
    expect(zsh.output).toContain(
      "_values 'command' acquire bootstrap completions diag dev doctor harness help list missing outdated repo version"
    )
    expect(bash.output).toContain('compgen -W "educate audit conform"')
    expect(bash.output).toContain(
      'compgen -W "acquire bootstrap completions diag dev doctor harness help list missing outdated repo version --help --version"'
    )
