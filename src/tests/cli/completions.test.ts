...
describe('[ki completion]', () => {
  test('renders zsh and bash completion scripts', async () => {
    ...
    expect(zsh.output).toContain(
      "_values 'command' acquire agora bootstrap cleanup completion dev diag docs doctor handoffs harness help list missing outdated repair repo search skill update version workspace"
    )
    ...
    expect(bash.output).toContain(
      'compgen -W "acquire agora bootstrap cleanup completion dev diag docs doctor handoffs harness help list missing outdated repair repo search skill update version workspace --help --version"'
    )
