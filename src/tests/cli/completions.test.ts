...
describe('[ki completion]', () => {
  test('renders zsh and bash completion scripts', async () => {
    ...
    expect(zsh.output).toContain("_describe -t ki-register-commands 'register command' commands")
    ...
    expect(bash.output).toContain('compgen -W "audit conform educate init plan skill upgrade"')
    expect(bash.output).toContain('compgen -W "add list"')
    expect(bash.output).toContain(
      'compgen -W "acquire agora bootstrap cleanup completion dev diag docs doctor harness help list missing outdated register repair repo search skill trades update version workspace --help --version"'
    )
