...
describe('[ki completion]', () => {
  test('renders zsh and bash completion scripts', async () => {
    ...
    expect(zsh.output).toContain(
      "_values 'command' acquire agora bootstrap cleanup completion dev diag docs doctor harness help list missing outdated repair repo search skill trades update version workspace"
    )
