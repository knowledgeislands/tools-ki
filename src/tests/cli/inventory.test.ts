...
const repoCommands = ['audit', 'conform', 'diag', 'educate', 'list', 'plan', 'skill', 'upgrade']
...
describe('[ki command inventory]', () => {
  test('keeps runtime help and completion memberships aligned with the public command contract', async () => {
    ...
    expect(commandNames(repository.output)).toEqual(['audit', 'conform', 'list', 'plan', 'educate', 'skill', 'upgrade', 'diag'])
