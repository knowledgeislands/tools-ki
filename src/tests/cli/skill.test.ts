...
describe('[ki skill]', () => {
  ...
  describe('user scope', () => {
    test('activates portable and runtime-bound skills only for compatible configured agents', async () => {
      ...
      expect(portableAdded).toEqual({ exitCode: 0, output: 'ki skill add: linked ki-example for claude-code, chatgpt-codex\n' })
    ...
    test('removes stale managed links from every configured agent after compatibility narrows', async () => {
      ...
      expect(removed).toEqual({ exitCode: 0, output: 'ki skill remove: unlinked ki-example for claude-code, chatgpt-codex\n' })
    ...
    test('links and declares a user skill, then unlinks and undeclares it', async () => {
      ...
      expect(repeated).toEqual({ exitCode: 0, output: 'ki skill remove: no KI-managed link for ki-example for claude-code\n' })
    })
  ...
  describe('repository scope', () => {
    test('rejects missing, malformed, and unsupported repository runtime declarations before mutation', async () => {
      const run = async (configuration: string): Promise<{ readonly output: string; readonly declared: string }> => {
        ...
        await box.run('ki bootstrap')
        const result = await box.run(`ki repo skill add ki-example --repo ${box.project.path}`)
    ...
    test('intersects repository and skill runtimes before linking or declaring', async () => {
      ...
      const refused = await box.run(`ki repo skill add ki-example --repo ${box.project.path}`)
    ...
    test('rejects non-directory repositories and missing or symbolic repository configuration files', async () => {
      ...
      const file = await box.run(`ki repo skill add ki-example --repo ${box.root.path}/not-a-repository`)
      const nonexistent = await box.run(`ki repo skill add ki-example --repo ${box.root.path}/does-not-exist`)
      const absent = await box.run(`ki repo skill add ki-example --repo ${missing}`)
      const symbolic = await box.run(`ki repo skill add ki-example --repo ${linked}`)
    ...
    test('links and declares a repository skill, then removes and undeclares it', async () => {
      ...
      expect(added).toEqual({ exitCode: 0, output: `ki repo skill add: linked ki-example into ${projectRoot} for claude-code\n` })
      expect(linkStat.isSymbolicLink()).toBe(true)
      expect(configAfterAdd).toContain('["example/harness:ki-example"]')

      const removed = await box.run(`ki repo skill remove ki-example --repo ${box.project.path}`)
      const configAfterRemove = await box.project.read('.ki-config.toml')
      expect(removed).toEqual({ exitCode: 0, output: `ki repo skill remove: removed ki-example in ${projectRoot} for claude-code\n` })
      await expect(lstat(link)).rejects.toThrow()
      expect(configAfterRemove).not.toContain('["example/harness:ki-example"]')

      const repeated = await box.run(`ki repo skill remove ki-example --repo ${box.project.path}`)
    ...
    test('refuses to remove a foreign repository skill directory', async () => {
      ...
      await box.run(`ki repo skill add ki-example --repo ${box.project.path}`)
      await unlink(link)
      await mkdir(link)
    ...
    test('does not duplicate a declaration and preserves a following table when removing it', async () => {
      ...
      const repeated = await box.run(`ki repo skill add ki-example --repo ${box.project.path}`)
      const removed = await box.run(`ki repo skill remove ki-example --repo ${box.project.path}`)
