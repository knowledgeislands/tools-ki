...
describe('[ki --help]', () => {
  test('prints root and nested command help through universal --help', async () => {
    ...
    const agoraAudit = await box.run('ki agora audit --help')
    const agoraInspect = await box.run('ki agora inspect --help')
    const agoraOpen = await box.run('ki agora open --help')
    const agoraReference = await box.run('ki agora reference --help')
    const agoraReferenceSet = await box.run('ki agora reference set --help')
    const vscode = await box.run('ki manage vscode --help')
    ...
    expect(agora.output).toContain('audit declared Agora health')
    expect(agora.output).toContain('inspect one local editor projection for Agora drift')
    expect(agora.output).toContain('open one Agora through an explicit local target')
    expect(agoraAudit.output).toContain('Usage: ki agora audit [options] [agora]')
