...
`
    ...
    test('materialises a vendored symlink into a real file at its resolved target', async () => {
      const box = await sandbox()
      const { payload, sha256 } = makeHarnessArchive({
        'skills/ki-example/SKILL.md': '---\nname: ki-example\nki-depends-on: []\n---\n',
        'skills/ki-skills/scripts/shared/checker.ts': 'export const checker = 1\n',
        'skills/ki-example/scripts/vendored/ki-skills/checker.ts': {
          type: '2',
          linkname: '../../../../ki-skills/scripts/shared/checker.ts'
        }
      })
      await box.config.write(
        'ki/config.toml',
...
`
      ...
      const materialised = `${box.data.path}/ki/harnesses/example/harness/skills/ki-example/scripts/vendored/ki-skills/checker.ts`
      expect((await lstat(materialised)).isSymbolicLink()).toBe(false)
      expect(await readFile(materialised, 'utf-8')).toBe('export const checker = 1\n')
    })

    test('refuses a vendored symlink whose target escapes the payload root', async () => {
      const box = await sandbox()
      const { payload, sha256 } = makeHarnessArchive({
        'skills/ki-example/SKILL.md': '---\nname: ki-example\nki-depends-on: []\n---\n',
        'skills/ki-example/scripts/vendored/legacy.ts': { type: '2', linkname: '../../../../../../etc/passwd' }
      })
      await box.config.write(
        'ki/config.toml',
        `[harnesses]
...
`
      ...
      expect(result.exitCode).toBe(1)
      expect(result.output).toContain('vendored symlink target escapes its payload')
    })

    test('refuses a vendored symlink whose target does not exist in the archive', async () => {
      const box = await sandbox()
      const { payload, sha256 } = makeHarnessArchive({
        'skills/ki-example/SKILL.md': '---\nname: ki-example\nki-depends-on: []\n---\n',
        'skills/ki-example/scripts/vendored/legacy.ts': { type: '2', linkname: 'missing.ts' }
      })
      await box.config.write(
        'ki/config.toml',
        `[harnesses]
...
`
      ...
      expect(result.exitCode).toBe(1)
      expect(result.output).toContain('vendored symlink target does not exist in the archive')
    })

    test('refuses a vendored symlink whose target is itself a symlink entry', async () => {
      const box = await sandbox()
      const { payload, sha256 } = makeHarnessArchive({
        'skills/ki-example/SKILL.md': '---\nname: ki-example\nki-depends-on: []\n---\n',
        'skills/ki-example/scripts/vendored/a.ts': { type: '2', linkname: 'b.ts' },
        'skills/ki-example/scripts/vendored/b.ts': { type: '2', linkname: 'a.ts' }
      })
      await box.config.write(
        'ki/config.toml',
        `[harnesses]
...
`
      ...
      expect(result.exitCode).toBe(1)
      expect(result.output).toContain('vendored symlink target must not itself be a symlink')
    })
