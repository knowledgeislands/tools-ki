...
vi.mock('node:fs/promises', async (importOriginal) => {
  ...
  return {
    ...
    writeFile: (...arguments_: Parameters<typeof original.writeFile>) => {
      if (writeFailure.enabled && String(arguments_[0]).endsWith('/kep.toml'))
        return Promise.reject(new Error('write failure'))
...
describe('[ki acquire chatgpt import]', () => {
  ...
  test('rejects malformed metadata and unsafe capture trees', async () => {
    ...
    const importCapture = (): Promise<CommandResult> =>
      box.run(`ki acquire chatgpt import ${capture.path} --output ${output}`)
...
`)
  ...
  test('rejects missing capture elements and unsafe output locations', async () => {
    ...
    const missingCaptureDirectory = await box.run(
      `ki acquire chatgpt import ${join(box.root.path, 'missing')} --output ${output}`
    )
  ...
  test('removes a partially written package after an output error', async () => {
    ...
    await expect(box.run(`ki acquire chatgpt import ${capture.path} --output ${output}`)).rejects.toThrow(
      'write failure'
    )
  ...
  test('rejects symbolic captures, invalid top-level types and unsafe names', async () => {
    ...
    expect(
      (await box.run(`ki acquire chatgpt import ${join(box.root.path, 'capture-link')} --output ${output}`)).output
    ).toContain('capture-directory must not be a symbolic link')
  })
...
describe('[ki space acquire chatgpt import]', () => {
  test('stages a capture content-addressably in the current repository Harbour', async () => {
    ...
    const second = await box.run(`ki space acquire chatgpt import ${capture.path}`)
    const packageId = /Package: kep:sha256:([a-f0-9]{64})/.exec(first.output)?.[1]

    expect(first.exitCode).toBe(0)
    expect(first.output).toContain('Acquisition staged:')
    expect(second.output).toContain('Acquisition checkpoint:')
    expect(packageId).toBeDefined()
    expect(await box.project.read(`+/_ACQUIRE/chatgpt/${packageId}/kep.toml`)).toContain(
      `package_id = "kep:sha256:${packageId}"`
    )
  })

  test('requires a KI repository and Harbour even for a dry run', async () => {
    const box = await sandbox()
    const capture = await makeCapture(box.root.path)
  ...
  test('plans a valid capture without writing a Harbour package', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.output).toContain('Acquisition plan:')
    expect(result.output).toContain('Dry run: no files written.')
    expect(
      await lstat(join(box.project.path, '+/_ACQUIRE/chatgpt'))
        .then(() => true)
        .catch(() => false)
    ).toBe(false)
  })
  ...
  test('rejects unsafe or mismatched existing Harbour stages', async () => {
    ...
    const packageId = /Package: kep:sha256:([a-f0-9]{64})/.exec(staged.output)?.[1] ?? 'missing-package-id'
    expect(packageId).not.toBe('missing-package-id')
    const stage = join(box.project.path, '+/_ACQUIRE/chatgpt', packageId)

    await rm(stage, { recursive: true })
    await box.project.write(`+/_ACQUIRE/chatgpt/${packageId}`, 'unsafe\n')
    expect((await box.run(`ki space acquire chatgpt import ${capture.path}`)).output).toContain(
      'existing acquisition stage is not a safe directory'
    )

    await rm(stage)
    await box.project.mkdir(`+/_ACQUIRE/chatgpt/${packageId}`)
    expect((await box.run(`ki space acquire chatgpt import ${capture.path}`)).output).toContain(
      'existing acquisition stage does not match the captured payload'
    )

    await rm(stage, { recursive: true })
    await box.project.write(`+/_ACQUIRE/chatgpt/${packageId}/kep.toml`, 'package_id = "kep:sha256:other"\n')
    expect((await box.run(`ki space acquire chatgpt import ${capture.path}`)).output).toContain(
      'existing acquisition stage does not match the captured payload'
    )
  })
