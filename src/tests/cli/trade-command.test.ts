...
const handoffsTable = 'knowledgeislands/ki-agentic-harness:ki-handoffs'
...
describe('[ki trades]', () => {
  test('adds, lists, checks, and removes one typed directional route without changing repository identity', async () => {
    ...
    const added = await box.run(['ki', 'trades', 'routes', 'add', receiverHome, '--direction', 'export', '--kind', 'work'])
    const listed = await box.run('ki trades routes list')
    const checked = await box.run(['ki', 'trades', 'routes', 'check', receiverHome, '--direction', 'export', '--kind', 'work'])
    const removed = await box.run(['ki', 'trades', 'routes', 'remove', receiverHome, '--direction', 'export', '--kind', 'work'])

    expect(added).toEqual({ exitCode: 0, output: `ki trades routes add: export work ${sourceHome} -> ${receiverHome}\n` })
    expect(listed).toEqual({ exitCode: 0, output: `ki trades routes list\nRepository: ${sourceHome}\nRoutes:\n  export work ${receiverHome} [active]\n` })
  ...
  test('creates, receives, displays, releases, and prunes a work trade while each command writes only its local repository', async () => {
    ...
    box.cd('receiver')
    const received = await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', id])
    await box.project.write(
      `receiver/+/_HANDOFFS/example/source/${id}.md`,
      (await box.project.read(`receiver/+/_HANDOFFS/example/source/${id}.md`)).replace('status: received', 'status: adopted\nadopted_as: "KI-RECEIVER-FND-001"')
    )
    box.cd('..')
    const listed = await box.run(['ki', 'trades', 'list', '--repo', receiverHome, '--direction', 'inbound', '--status', 'adopted', '--kind', 'work'])
    const shown = await box.run(['ki', 'trades', 'show', id])
    const released = await box.run(['ki', 'trades', 'release', id])
    box.cd('receiver')
    const pruned = await box.run(['ki', 'trades', 'prune', id])

    expect(created.output).toBe(`ki trades new: created ${id} for example/receiver\n`)
    expect(received).toEqual({ exitCode: 0, output: `ki trades receive\n  received ${id}\n` })
    expect(listed).toEqual({ exitCode: 0, output: `ki trades list\n  ${receiverHome} inbound ${id} [work, adopted] Route contract\n` })
    expect(shown.output).toContain(`Repository: ${sourceHome} [outbound]\n${outbound.trimEnd()}`)
    expect(released).toEqual({ exitCode: 0, output: `ki trades release: released ${id}\n` })
    expect(pruned).toEqual({ exitCode: 0, output: `ki trades prune: pruned ${id}\n` })
  ...
  test('permits retained knowledge to release and prune, but refuses retained work', async () => {
    ...
    const id = /HND-[0-9a-f-]+/u.exec(created.output)?.[0] as string
    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'knowledge', '--id', id])
    const path = `receiver/+/_HANDOFFS/example/source/${id}.md`
    ...
    const invalidId = /HND-[0-9a-f-]+/u.exec(invalid.output)?.[0] as string
    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', invalidId])
    const invalidPath = `receiver/+/_HANDOFFS/example/source/${invalidId}.md`
    await box.project.write(
      invalidPath,
      (await box.project.read(invalidPath)).replace('status: received', 'status: retained\nretained_as: "Knowledge/Local/Note"')
    )
    box.cd('..')
    expect((await box.run(['ki', 'trades', 'release', invalidId])).output).toContain('permits retained only for knowledge trades')
  })
  ...
  test('reports malformed route declarations and every registered-estate route state', async () => {
    ...
    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source'))
    expect(await box.run('ki trades routes list')).toEqual({
      exitCode: 0,
      output: `ki trades routes list\nRepository: ${sourceHome}\nRoutes:\n  none\n`
    })
    expect(await box.run('ki trades routes check')).toEqual({ exitCode: 0, output: 'ki trades routes check\n  none\n' })

    await box.project.write('.ki-config.toml', '[not valid TOML\n')
    expect((await box.run('ki trades routes list')).output).toContain('must be valid TOML')

    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source').replace(`repository = "${sourceHome}"\n`, ''))
    expect((await box.run('ki trades routes list')).output).toContain('.repository must use canonical HTTPS GitHub repository form')
    ...
    expect((await box.run('ki trades routes list')).output).toContain('has unrecognised key unknown')

    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source').replace('work = []', 'other = []\nwork = []'))
    expect((await box.run('ki trades routes list')).output).toContain('has unrecognised trade kind other')
    ...
    for (const routes of [[sourceHome], [receiverHome, receiverHome], [receiverHome, 'https://github.com/aaa/first']]) {
      ...
      expect((await box.run('ki trades routes list')).exitCode).toBe(2)
    }

    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source').replace('work = []', 'work = [1]'))
    expect((await box.run('ki trades routes list')).output).toContain('must be a canonical HTTPS GitHub repository URL array')

    const repositoryOnly = repositoryConfiguration('example/source').split(`["${handoffsTable}".exports_to]`)[0] as string
    await box.project.write('.ki-config.toml', repositoryOnly)
    expect((await box.run('ki trades routes list')).output).toContain(`does not declare [${handoffsTable}]`)
    await box.project.write('.ki-config.toml', `${repositoryOnly}["${handoffsTable}"]\n`)
    expect((await box.run('ki trades routes list')).output).toContain('.exports_to must be a table')
    await box.project.write('.ki-config.toml', `${repositoryOnly}["${handoffsTable}"]\n\n[after]\nvalue = true\n`)
    expect((await box.run(`ki trades routes add ${receiverHome} --direction export --kind work`)).exitCode).toBe(0)
    expect(await box.project.read('.ki-config.toml')).toContain('[after]\nvalue = true')
    await box.project.write(
      '.ki-config.toml',
      `"${handoffsTable}" = { exports_to = { work = [], knowledge = [] }, imports_from = { work = [], knowledge = [] } }\n${repositoryOnly}`
    )
    expect((await box.run(`ki trades routes add ${receiverHome} --direction export --kind work`)).output).toContain('does not declare')
    ...
    expect((await box.run('ki trades routes check')).output).toContain(`${missingHome}: missing repository`)

    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source', { work: [receiverHome] }))
    await box.project.write('receiver/.ki-config.toml', repositoryConfiguration('example/receiver'))
    expect((await box.run('ki trades routes check')).output).toContain(`${receiverHome}: nonreciprocal`)

    await box.project.write('receiver/.ki-config.toml', repositoryConfiguration('example/receiver', {}, { work: [sourceHome] }))
    await box.project.write('duplicate/.ki-config.toml', repositoryConfiguration('example/receiver', {}, { work: [sourceHome] }))
    expect((await box.run('ki trades routes check')).output).toContain(`${receiverHome}: ambiguous repository`)

    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source'))
    expect((await box.run(`ki trades routes add ${sourceHome} --direction export --kind work`)).output).toContain('must differ from the local repository')
    expect((await box.run(`ki trades routes remove ${receiverHome} --direction export --kind work`)).output).toContain('is not declared locally')
  ...
  test('covers import-route mutation and command filters without changing peer configuration', async () => {
    ...
    expect(await box.run('ki trades show HND-00000000-0000-0000-0000-000000000000')).toEqual({
      exitCode: 2,
      output: 'ki: error: trade HND-00000000-0000-0000-0000-000000000000 was not found in the registered repository estate\n'
    })
    box.cd('receiver')
    expect(await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work'])).toEqual({ exitCode: 0, output: 'ki trades receive\n' })
    const removed = await box.run(['ki', 'trades', 'routes', 'remove', sourceHome, '--direction', 'import', '--kind', 'knowledge'])
    const added = await box.run(['ki', 'trades', 'routes', 'add', sourceHome, '--direction', 'import', '--kind', 'knowledge'])
    const selected = await box.run(['ki', 'trades', 'routes', 'check', sourceHome, '--direction', 'import', '--kind', 'knowledge'])
    const absent = await box.run(['ki', 'trades', 'routes', 'check', home('example/absent')])
    const badDirection = await box.run(`ki trades routes check ${sourceHome} --direction sideways`)
    const badKind = await box.run(`ki trades routes check ${sourceHome} --kind other`)
    const badListDirection = await box.run('ki trades list --direction sideways')
    const badListRepository = await box.run('ki trades list --repo example/source')
    const badId = await box.run('ki trades show HND-invalid')
    ...
    expect(badId.output).toContain('trade id must use HND-')
  })
  ...
  test('rejects malformed outbound envelopes observed by the receiver', async () => {
    ...
    const cases: readonly [string, string][] = [
      ...
      [outbound.replace(`id: ${id}`, 'id: HND-invalid'), 'trade id must use HND-'],
    ...
    for (const [contents, message] of cases) {
      ...
      expect((await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', id])).output).toContain(message)
    }

    await box.project.write(path, outbound.replace('receiver: example/receiver', 'receiver: example/other'))
    expect((await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', id])).output).toContain(
      'does not match the active work trade route'
    )
    await box.project.write(path, outbound)
    const missing = await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', 'HND-00000000-0000-0000-0000-000000000000'])
  ...
  test('receives all matching trades, reports existing copies, and filters distinct records', async () => {
    ...
    await box.project.write('-/_HANDOFFS/not-an-owner', 'not a directory')
    await box.project.write('-/_HANDOFFS/example/not-a-repository', 'not a directory')
    const outboundList = await box.run('ki trades list --direction outbound')
    box.cd('receiver')
    const received = await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work'])
    const repeated = await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work'])
    box.cd('..')
    const shown = await box.run(['ki', 'trades', 'show', firstId])
  ...
  test('reports missing, invalid, and unregistered user configuration before trade mutation', async () => {
    ...
    expect((await unbootstrapped.run('ki trades routes list')).output).toContain('ki environment is not bootstrapped')

    const box = await sandbox()
    const source = await realpath(box.project.path)
    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source'))
    await box.config.write('ki/config.toml', 'not valid TOML')
    expect((await box.run('ki trades routes list')).output).toContain('ki configuration is invalid')
  ...
  test('validates receiver-only status fields, payload immutability, and lifecycle evidence', async () => {
    ...
    const id = /HND-[0-9a-f-]+/u.exec(created.output)?.[0] as string
    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', id])
    const path = `receiver/+/_HANDOFFS/example/source/${id}.md`
    ...
    const releaseWith = async (status: string) => {
      ...
      box.cd('..')
      const result = await box.run(['ki', 'trades', 'release', id])
      box.cd('receiver')
      return result
    }
    ...
    const cases: readonly [string, string][] = [
      ...
      ['status: received\nsuperseded_by: "HND-other"', 'permits superseded_by only for status superseded']
    ]
    ...
    const changedId = /HND-[0-9a-f-]+/u.exec(changed.output)?.[0] as string
    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', changedId])
    const changedPath = `receiver/+/_HANDOFFS/example/source/${changedId}.md`
    await box.project.write(
      changedPath,
    ...
    box.cd('..')
    expect((await box.run(['ki', 'trades', 'release', changedId])).output).toContain('does not preserve the sender payload')

    const knowledge = await box.run(newTrade('knowledge'))
    const knowledgeId = /HND-[0-9a-f-]+/u.exec(knowledge.output)?.[0] as string
    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'knowledge', '--id', knowledgeId])
    const knowledgePath = `receiver/+/_HANDOFFS/example/source/${knowledgeId}.md`
    ...
    box.cd('..')
    expect((await box.run(['ki', 'trades', 'release', knowledgeId])).output).toContain('permits adopted only for work trades')
    box.cd('receiver')
    await box.project.write(knowledgePath, knowledgeInbound.replace('status: received', 'status: retained'))
    box.cd('..')
    expect((await box.run(['ki', 'trades', 'release', knowledgeId])).output).toContain('requires retained_as for status retained')

    const superseded = await box.run(newTrade('work'))
    const supersededId = /HND-[0-9a-f-]+/u.exec(superseded.output)?.[0] as string
    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', supersededId])
    const supersededPath = `receiver/+/_HANDOFFS/example/source/${supersededId}.md`
    await box.project.write(
      supersededPath,
      (await box.project.read(supersededPath)).replace(
        'status: received',
        'status: superseded\nrationale: "newer trade"\nsuperseded_by: "HND-00000000-0000-0000-0000-000000000000"'
      )
    )
    box.cd('..')
    expect((await box.run(['ki', 'trades', 'release', supersededId])).exitCode).toBe(0)
  })
  ...
  test('rejects absent, premature, foreign, and ambiguous local lifecycle evidence', async () => {
    ...
    const created = await box.run(newTrade('work'))
    const id = /HND-[0-9a-f-]+/u.exec(created.output)?.[0] as string
    expect((await box.run(['ki', 'trades', 'release', 'HND-00000000-0000-0000-0000-000000000000'])).output).toContain('was not found in the current repository')
    expect((await box.run(['ki', 'trades', 'release', id])).output).toContain('receiver has not recorded an inbound trade')

    box.cd('receiver')
    await box.run(['ki', 'trades', 'receive', '--from', sourceHome, '--kind', 'work', '--id', id])
    expect((await box.run(['ki', 'trades', 'prune', id])).output).toContain('cannot be pruned while receiver status is received')
    ...
    expect((await box.run(['ki', 'trades', 'prune', id])).output).toContain('before sender release is observable')

    box.cd('..')
    const duplicate = await box.project.mkdir('duplicate')
    await box.project.write('duplicate/.ki-config.toml', repositoryConfiguration('example/receiver', {}, { work: [sourceHome] }))
    await box.config.write('ki/config.toml', localConfiguration([source, receiver, duplicate]))
    expect((await box.run(['ki', 'trades', 'release', id])).output).toContain('unavailable or ambiguous')

    await box.config.write('ki/config.toml', localConfiguration([source, receiver]))
    await box.project.write('.ki-config.toml', repositoryConfiguration('example/other', { work: [receiverHome] }))
    expect((await box.run(['ki', 'trades', 'release', id])).output).toContain('not owned by the current repository')

    await box.project.write('.ki-config.toml', repositoryConfiguration('example/source', { work: [receiverHome] }))
    box.cd('receiver')
    await box.project.write('receiver/.ki-config.toml', repositoryConfiguration('example/other-receiver', {}, { work: [sourceHome] }))
    expect((await box.run(['ki', 'trades', 'prune', id])).output).toContain('not addressed to the current repository')
  })
})
