...
describe('ki skill', () => {
  ...
  describe('user scope', () => {
    test('links and declares a user skill, then unlinks and undeclares it', async () => {
      ...
      const added = await box.run(['skill', 'user', 'add', 'ki-example'])
      ...
      const removed = await box.run(['skill', 'user', 'remove', 'ki-example'])
      ...
      const repeated = await box.run(['skill', 'user', 'remove', 'ki-example'])
    ...
    test('re-points a divergent KI-managed user link only under --replace', async () => {
      ...
      const refused = await box.run(['skill', 'user', 'add', 'ki-example'])
      ...
      const replaced = await box.run(['skill', 'user', 'add', 'ki-example', '--replace'])
    ...
    test('refuses to adopt a foreign user skill directory', async () => {
      ...
      const guarded = await box.run(['skill', 'user', 'add', 'ki-example'])
  ...
  describe('repository scope', () => {
    test('links and declares a repository skill, then removes and undeclares it', async () => {
      ...
      const added = await box.run(['skill', 'repo', 'add', 'ki-example', '--repo', box.project.path])
      ...
      const removed = await box.run(['skill', 'repo', 'remove', 'ki-example', '--repo', box.project.path])
      ...
      const repeated = await box.run(['skill', 'repo', 'remove', 'ki-example', '--repo', box.project.path])
