    ...
    .addCommand(
      new Command('conform')
        ...
        .action(async (options: { repo?: string; skill?: string; dryRun?: boolean }) => {
          const { repository, skills } = await resolveSkills(context, options)
          const conformed = await Promise.all(skills.map(async (skill) => ({ skill, conform: await runSkillConform(repository.root, skill) })))
          ...
          const reaudited = await Promise.all(conformed.map(async ({ skill, conform }) => ({ conform, audit: await runSkillAudit(repository.root, skill) })))
