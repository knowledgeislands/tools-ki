...
const createUninstallCommand = (context: KiContext): Command =>
  new Command('uninstall')
    ...
    .action(async (value: string) => {
      ...
      await uninstallHarness(context.paths.data, resolved.identifier)
      await recordInstalledHarness(context.paths.config, resolved.identifier, false)
      context.stdout.write(`uninstalled ${resolved.identifier}\n`)
    })
