...
const refreshHarnesses = async (
  ...
  for (const harness of [...harnesses].sort((left, right) => left.id.localeCompare(right.id))) {
    ...
    if (dryRun) {
      lines.push(`${harness.id}: would refresh`)
      continue
    }
...
const updateExecutable = async (context: KiContext, dryRun: boolean): Promise<string> => {
  ...
  if (dryRun) return `CLI executable: would update with ${receipt.installer}`
...
export const createUpdateCommand = (context: KiContext): Command =>
  new Command('update')
    ...
    .option('--dry-run', 'validate update targets without downloading or changing state')
    .option('--cli', 'update only the installer-managed CLI executable')
    .action(async (options: { dryRun?: boolean; cli?: boolean }) => {
      const lines = ['ki update']
      if (options.cli) {
        lines.push(await updateExecutable(context, Boolean(options.dryRun)))
      ...
      const refreshed = await refreshHarnesses(context, harnesses, Boolean(options.dryRun))
...
export const createUpgradeCommand = (context: KiContext): Command =>
  new Command('upgrade')
    .description('refresh uniquely resolved capabilities declared by the current repository')
    .option('--dry-run', 'validate repository capability updates without downloading or changing state')
    .action(async (options: { dryRun?: boolean }) => {
      ...
      const lines = ['ki upgrade']
      if (!selected.length) {
        lines.push('No declared capabilities.')
        context.stdout.write(`${lines.join('\n')}\n`)
        return
      }
      const refreshed = await refreshHarnesses(context, selected, Boolean(options.dryRun))
