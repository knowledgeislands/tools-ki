...
- Repository audit and conform now share phase and evidence progress, report phase timings that reconcile with the total, weight item bars by the catalogue's declared `cost`, and close their output tree only at the final summary. Terminal runs use stable per-skill rows with only the active skill expanded, while clean result and repository summary lines omit zero-valued diagnostic counters. Conform labels a post-write pass `re-audit` and skips it with an explicit message when no write or command was staged.
