...
- Repository audit and conform now share a compact receipt stream and one mutable activity row. The bar indicates activity without estimating completion from item count or declared `cost`; evidence-ready skills receive one full receipt before collapsing to a timed evidence receipt, queued skills stay hidden, and the operation receipt retains total elapsed time. Clean result and repository summary lines omit zero-valued diagnostic counters. Conform labels a post-write pass `re-audit` and skips it with an explicit message when no write or command was staged.
