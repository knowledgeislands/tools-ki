...
- **The pre-V1 release-marker disagreement is intentional** — `CHANGELOG.md` opens with `## [1.0.0] — in progress` because it records the V1 baseline rather than the individual `0.x` releases leading to it, while `package.json` and the Git tags track the shipped `0.x` version. `ki repo audit` therefore reports a standing `TOOL-RELEASE-MARKERS` WARN that both markers disagree, and that WARN is correct evidence about a deliberate transitional state. Do not silence it by bumping `package.json` to `1.0.0`, which would assert a release that has not happened, or by rewriting the changelog heading, which would discard the V1 baseline. It resolves when V1 actually ships.
- **Verify a portable claim against its specification** — when an assertion about a format, protocol, or standard leaves this repository, particularly in a trade, check it against that standard rather than against the parser or library that happens to implement it here. A permissive implementation reads as agreement and a strict one reads as prohibition, and neither is evidence about the standard. `smol-toml` accepting a construction says nothing about whether TOML permits it. The same trap runs the other way, in what a parser can be asked to enforce: where the grammar makes two constructions indistinguishable after parsing, `ki` states the rule and does not pretend to check it. `[skills.ki-trades.routes]` implicitly creates `skills.ki-trades`, so an explicitly declared root table and an omitted one parse identically — the standard requires the explicit declaration, and the parser accepts both rather than inventing a text-level check the format does not support.
