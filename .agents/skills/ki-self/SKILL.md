...
ki-depends-on: []
description: >
  Repository-local governance for tools-ki. Use when changing this CLI's bootstrap inventory, managed-skill activation or repair, harness discovery, native operation boundaries, or local output and release conventions. Keeps repository-specific standards auditable without promoting them prematurely into a portable Knowledge Islands skill.
...
Inspect the covered bootstrap, classification, and repair invariants against the rubric. Report the affected source and corrective action. Treat an audit failure as evidence to diagnose; do not restore a retired wrapper, hard-coded source path, or compatibility branch to make it disappear.

### Mode CONFORM

Apply only safe repository-local corrections. Preserve unrelated work, keep the bootstrap inventory authoritative in one source, and verify the affected CLI contract before making an explicit commit.
